...
本文档用 6 个场景演示 LocalAgent 的核心能力。示例数据均为虚构内容，可安全复现。
...
```bash
# 写入一条记忆
LA add "2026年7月决定为 LocalAgent 补充 examples 目录，方便新用户快速上手"

# 召回验证
LA search "examples 目录"
```
...
## 示例 2：按 Markdown 文件增加记忆，演示召回

适合导入项目笔记、日记、技术方案等长文档。文件会软链到 `data/kb/`，同时写入知识库（Cold 层）和记忆（Warm 层）。
...
> 此功能需要配置 `TAVILY_API_KEY`（[tavily.com](https://tavily.com) 提供免费额度）。其余功能不受影响。
...
| 环境 | 体验 |
|------|------|
| Apple Silicon Mac（16GB 内存） | 流畅，简单问答 3–8 秒 |
| Intel Mac（16GB 内存） | 可用，简单问答 10–20 秒 |
...
- `tavily` 行：仅联网搜索产生费用；纯本地模式下此项为 0
...
- 导入自己的 Markdown：`LA add-file ~/Documents/notes.md`
- 导入 ChatGPT 历史：`LA import-chatgpt conversations.json`
