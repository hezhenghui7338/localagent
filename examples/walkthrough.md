...
```bash
# 导入示例文档（仓库自带）
LA memory add-file examples/sample-project-notes.md

# 从知识库召回原文片段（Cold 层）
LA memory search "三层记忆架构" --knowledge

# 从记忆层召回提取的事实（Warm 层）
LA memory search "qwen3.5:4b"
```
...
LA memory add-file examples/sample-project-notes.md
LA memory search "三层记忆" --knowledge
LA workspace --cwd .
LA audit --since 7d
...
- 导入自己的 Markdown：`LA memory add-file ~/Documents/notes.md`
