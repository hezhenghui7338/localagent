...
> **目标**：用一条连贯的「虚构用户」故事，在约 **30 分钟**内亲手跑通 LocalAgent 的核心**用户故事**（对照 [PRD §2](../docs/PRD.md)）。  
> 每一步都给出**完整输入**与**预期输出**（示例数据均为虚构，可安全复现）。  
> 命令入口：`la` / `LA`（等价）。更短的 6 场景上手见 [walkthrough.md](walkthrough.md)。  
...
| # | 用户故事 | 本节 |
|---|----------|------|
| 1 | 一键装好、马上聊天（用户 / 开发者） | [§1](#1-一键安装与-hello用户--开发者) |
| 2 | 纯本地零成本，或用自有 API Key 也能跑 | [§2](#2-纯本地与自有-api-双路径) |
| 3 | 被 profile、跨会话记住 | [§3](#3-跨会话记忆--hot--warm--cold) |
| 4 | 导入 ChatGPT，更快认识你 | [§4](#4-chatgpt-导入加速认识你) |
| 5 | 本地文档进知识库并深度召回 | [§5](#5-本地文档-rag-深度召回) |
| 6 | 联网搜索 | [§6](#6-联网搜索--小模型也能用好网络) |
| 7 | 本地工具 + 危险命令拦截 | [§7](#7-本地工具--危险命令拦截) |
| 8 | 审计花费（Token / 费用） | [§8](#8-可审计--token--费用) |
| 9 | 进阶：写文件幻觉检测 + 时间优先召回 | [§9](#9-进阶写文件幻觉检测--时间优先召回) |

**叙事设定（虚构）**：你是「林晓」，在本机上用 LocalAgent 做个人 AI 中枢；偏好美式咖啡；2026-05 在深圳开会定了产品路线；2026-07 决定用 Mem0 做记忆引擎。
...
LA memory add "2026年5月，我在深圳开会讨论产品路线，结论是优先做本机个人 AI 中枢"
```
...
[chat] 调用 搜索记忆…
你叫林晓，喜欢美式咖啡（不喜欢拿铁）。
2026 年 5 月你在深圳开会讨论产品路线，结论是优先做本机个人 AI 中枢。
[via ollama/qwen3.5:4b]
```
...
```text
你> 对了，我的长期目标是把 LocalAgent 做成真正懂我的本机个人 AI 中枢。
助手> …（正常对话）
你> /q
```
...
> **对比点**：不是「语义最像就排前面」，而是**问题要求的时间域 × 记忆发生时间**对齐后再推理——这正是长期中枢答对「当时 vs 现在」的关键。
