...
| 6 | 写文件幻觉检测：明确路径后确认写入，不空口「已写入」 | [§6](#6-写文件幻觉检测) |
| 7 | 对话记忆与知识库：对话自动 / `add` / `rag add` / `rag ingest` / `import-chatgpt` | [§7](#7-对话记忆与知识库写入) |
...
| **Cold** | 文档原文片段（语义 + BM25） | `la rag add` → `la search --knowledge` |
...
| 手动一条 | `la add "…"` | 精确写入 |
| 文档提取 | `la rag add <path>` | 软链 + Cold 全文 |
| 目录同步 | `la rag ingest` | 扫描 `data/kb/` 全部文档 |
...
交互部分（chat 联网、Shell 确认、写文件）请按 §3 / §4 / §6 手动走一遍。
...
- [ ] 明确路径的写文件会弹出确认并真正写入  
- [ ] `add` / `rag add` / `rag ingest` / `import-chatgpt` 至少各成功一次  
