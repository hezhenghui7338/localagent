...
11. 【证据核对·必须遵守】使用工具结果（尤其是联网搜索）作答前，必须核对与用户请求一致的基础信息：
   ...
   - 地点：结果中的城市/地区必须与用户所说一致；不一致则不可套用
   - 出现【核对失败】【时效警告】或结果自带「过期」标注时：不可把过期内容当今日事实；可再调用一次 web_search（查询须含完整目标日期与地点），或明确告知证据不足
可用工具（以 JSON 格式请求）：
{tools}
...
def _prefetch_web_context(user_message: str) -> str:
    ...
    if search_output_has_freshness_warning(result):
        header = (
            ...
            "请勿把过期/未核实结果当作当前事实；"
            "可再调用 web_search（查询须含完整目标日期与地点），或明确告知证据不足。"
            "若仍作答，必须标注来源标题与完整链接。"
        )
    else:
        header = (
...
def _tool_followup_instruction(tool_name: str, result: str) -> str:
    ...
    if tool_name == "web_search" and search_output_has_freshness_warning(result):
        return (
            ...
            "若全部过期或不符：可再调用一次 web_search"
            "（query 必须包含完整目标日期与用户地点，例如「深圳 2026年7月15日 天气」）；"
            "若仍无可用证据，明确告知无法确认当前情况，"
            "禁止把过期结果当作今日事实播报。"
