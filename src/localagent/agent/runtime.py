...
9. run_shell / write_file 会先经用户确认；若工具结果为「用户拒绝」，如实告知并给出不执行该命令的替代建议，不要擅自重试同一危险操作
10. 回答简洁、准确；使用联网搜索（含预加载结果）作答时【必须标注来源】：在答复末尾列出所依据条目的标题与完整链接，便于用户核实。禁止只写「根据联网信息/预加载结果」而不给链接
11. 【证据核对·必须遵守】使用工具结果（尤其是联网搜索）作答前，必须核对与用户请求一致的基础信息：
   ...
   - 出现【核对失败】【时效警告】或结果自带「过期」标注时：不可把过期内容当今日事实；**必须先再调用一次 web_search 换查询重试**（天气用「城市 今天 天气预报」，勿写完整年份；禁止把歌词/教案/PDF 当天气证据）；仅当重试后仍无可用证据才可说明证据不足
12. 回答天气类问题时：按居住地/用户指定城市直接作答；末尾加一句「若要查其他城市的天气，直接告诉我城市名即可」
可用工具（以 JSON 格式请求）：
{tools}
...
def _prefetch_web_context(user_message: str) -> str:
    ...
    soft_weather = ""
    if is_weather_query(searchable):
        soft_weather = "天气答复末尾加一句：若要查其他城市，直接告诉我城市名即可。"
...
def _build_system_prompt(
    *,
    ...
    profile = load_core_profile().format_for_prompt()
    prompt = (
        f"{SYSTEM_PROMPT.format(tools=tools_desc, today=today_text)}\n\n{profile}"
    )
    if personal_context:
        prompt = f"{prompt}\n\n{personal_context}"
    if session_context:
    ...
    *,
    ...
    def _log_tool_decision(
        ...
        *,
        ...
        elif tool_name == "write_file" and arguments:
    ...
    for iteration in range(3):
        ...
        result = _truncate_for_llm(_gated_execute(tool_name, arguments))
        messages.append(ChatMessage(role="assistant", content=reply))
        messages.append(
            ChatMessage(
                role="user",
