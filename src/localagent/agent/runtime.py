...
def _prefetch_aware_context(user_message: str) -> str:
    ...
    try:
        ...
        return ""
    try:
        card = retrieve_aware_context(user_message, since_hours=24, limit=10)
    except Exception:
        return ""
    if not (card or "").strip():
        return ""
    # Cap for chat budget (~400 tokens ≈ 800–1200 Chinese chars)
    clipped = card if len(card) <= 1200 else card[:1200] + "\n…"
    return (
