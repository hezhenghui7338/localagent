...
**定位**：**本地个人 AI 助手**——一键可用；算力 / 网络 / 工具归你；持久记忆，越来越懂你；能力可扩展（本机工具 + RAG + 可选增强）。
...
LocalAgent 不是「又一个 Chat 客户端」，而是跑在本机上的**个人 AI 助手**。模型是「算力供应商」，LocalAgent 是「会话与记忆 owner」。身份、记忆与审计数据始终留本机。
...
| 6b | 用户 | 一键总结本地文档（最多三句话 + 带章节/页索引的要点） | 3 分钟读懂；默认进入文档对话；默认不入库 | `la summarize` / `sum>`；`--keep` / `/keep` 可选入库 | §6.2 |
| 6c | 用户 | 从信任信源（默认 BestBlogs RSS）嗅探今日资讯，交互浏览简报并精读几篇 | 替代每天手动刷几百篇 | `la news sync/brief`；TTY ↑↓/`o`/`r`；早 8 点可配置 sync；入 LA 就绪通知 | §6.2 |
| 6d | 用户 | 一键润色邮件/朋友圈/简历/商务对话草稿 | 识别场合态度后改写；主推进剪贴板，可直接粘贴发出 | `la polish` / `/polish`；`--scene` / `--tone` / `--no-copy` | §6.2 |
...
2. **实践胜于旁观**：「书读百遍其义自见」不会自动发生；听过一万遍别人谈论 AI，不如一键下载 LA，亲自调试——本机就能开发、使用的个人 AI 助手。  
3. **本地实验台**：看到新方案或新技术时，不妨先在本机助手上试一遍，亲自看能做什么、局限在哪。  
4. **只摘低垂成熟的果实**：不引入很重的、无法控制的、成本高昂的技术栈；保证易用，不设置障碍阻止用户使用 AI，相反要不遗余力消除障碍。  
5. **只做一件事——本地个人 AI 助手**：一切围绕本机可跑通展开。LA **不拒绝联网**，也不拒绝最新技术，但把**数据留在本地**，且**本地也能完整跑通**。
...
### 6.2 记忆 · ChatGPT · RAG · 日常三剑客（故事 4–6 / 6b–6d）
...
- **6b 一键总结**：`la summarize <path>` 输出 1～3 句 + 〔§/p.〕要点；TTY 默认进 `sum>`；默认不写 kb；`--keep` / `/keep` 后可检索；不追问入库
- **6c 新闻嗅探**：`la news sync` 拉取 BestBlogs RSS；TTY 下 `la news brief` 可 ↑↓ 浏览、`o` 打开浏览器、`r` 精读深聊；`schedule on/off` 控制早 8 点自动 sync；入 chat 可提示就绪
- **6d 一键润色**：`la polish` / `/polish` 输出识别 Brief + 主推/备选；默认主推进剪贴板；`--no-copy` 可关；简历场景不编造原文没有的数字
...
**尚未做**：工作区 watcher 增量索引、外部任务源；新闻 OpenAPI / 个人 OPML（MVP 仅 BestBlogs RSS）。`LA audit --report *.html` 已支持。
