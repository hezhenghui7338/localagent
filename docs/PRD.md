...
**下一周期**：Aware 扩展（apps / wechat / calendar / email）、与 `rag` 更紧协同、外部任务源。
...
- **Daily Actions + 数据层**：欢迎横幅可见今日信号与 Hot/Warm/Cold/Aware 高层次；`la status` / `/status` 含完整数据层盘点与召回优先级说明；托管待办可 `done`/`dismiss`/`snooze`
