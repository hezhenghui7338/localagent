...
| 2 | **会不会主动搞清楚我想干什么** | 意图模糊时**主动追问** 1–2 个关键问题，确认后再调用工具；宁可多问一句，也不基于错误假设改文件、跑命令 | ✅ `LA_INTENT_CLARIFY` 预检 + 幻觉检测兜底 |
| 3 | **能不能长期、多层次地记住我** | Hot（核心画像）/ Warm（长期记忆）/ Cold（文档原文）分层；配合强大的 **Hindsight** 记忆引擎，跨会话 JIT 召回「懂你」的上下文 | ✅ 分层记忆栈 + Hindsight；持续优化召回 |
| 4 | **记忆从哪来、能不能全方位认识我** | 记忆引擎输入支持 **ChatGPT 历史对话**、个人文档、LocalAgent 对话；多源冷启动 → pending 确认 → Hindsight 长期记忆，全方位记住用户 | ✅ `import-chatgpt` / `add-file` / chat 提取同一管线 |
...
**一句话**：LocalAgent = **完全本地** + **主动意图澄清** + **长期多层次记忆（Hindsight）** + **多源记忆输入** + **本机 · 画像 · 联网三层打通**。
...
3. **长期多层次记忆**：Hot / Warm / Cold + Hindsight；助手记住「我是谁、我说过什么」，跨模型不丢身份，但不喧宾夺主
...
- **Warm**：结构化长期记忆，优先接入强大的 **Hindsight** 引擎（Retain → 多路 Recall → Reflect → Consolidation）；亦可回退 JSON memory
