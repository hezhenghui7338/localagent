...
| 语义召回 | `LA search "记忆引擎选型"` | Mem0 语义召回 |
| 时间感知 | `LA search "2026年5月 决定"` | 按发生时间重排序 |
| 标签浏览 | `LA memories --tag 决策` | 结构化查询 |
| 跨记忆推理 | `LA reflect "选型经历了什么变化？"` | Mem0 search + LLM reflect |
...
命令:
  ...
  <command>
    ...
    memory-status  诊断 Warm 层记忆后端（Mem0 / JSON）
    rebuild-memory
                   清空记忆后强制重建 kb/ 索引
    ...
    reflect        <query> 跨记忆推理（Mem0 reflect）
