...
| **Aware**（本机感知，需授权） | `la aware`（当前状态 + 近 3h 动态 → `aware>`）· `grant apps` 前台/正在播放 + 今日输入活跃时长估算（不记按键内容）· Episode/洞察 · `la chat` 相关时注入 |
...
3. **Actions Automated** — Shell / 写文件 / 工作区；`la summarize` · `la news` · `la polish`；定时简报；执行前确认、危险硬拦、办完有回执；`la status` / `/status` 看今日信号与数据层  
...
日常：
  ...
  la aware [--detail] [--since 1w] | tick | grant   # 本机感知（默认智能总结 / --detail 明细）
