...
> **完全本地 · 长期懂你 — 打通本机、用户画像与互联网，真正可用。**

LocalAgent（`LA`）不是又一个 Chat 客户端，而是跑在你本机上的**个人 AI**。核心叙事：

1. **完全本地化** — 默认 Ollama `qwen3.5:4b`，对话、记忆、检索与执行全链路可纯本地跑通，身份与数据不出本机  
2. **长期、多层次记忆** — Hot / Warm / Cold 分层，真正「懂你」的助手  
3. **记忆从哪来** — **ChatGPT 历史对话**与 **LA 日常对话**入库；个人文档走 **RAG 知识库**（不提取记忆），配合 **Mem0** 记住用户  
4. **真正可用** — 联网查询 + 本地 Shell，打通**电脑本地 · 用户画像 · 互联网**三层能力  
...
| 记忆只能靠本轮聊天 | ChatGPT 历史 + LA 对话冷启动；文档走独立 RAG |
| 不会搜网、不会跑命令 | **联网搜索** + **本地 Shell**，本机 · 画像 · 联网打通 |
| 声称「已写入」却没真改文件 | **幻觉检测** + 执行前确认，写文件更可靠 |
...
- **Mem0 + 对话记忆**：ChatGPT 历史与 LA 日常对话 → Warm 记忆；文档经 `LA rag` 进 Cold 知识库
...
- **写文件幻觉检测**：模型声称「已写入」却未调用 `write_file` 时，自动重试或明确报错
- **文档知识库**：软链导入个人文件，Chroma + BM25 混合检索
- **多模型对话**：Ollama / OpenRouter / Cursor 统一入口，`auto` 模式按优先级自动降级
- **可审计**：Token/费用、Agent 行为（shell/写文件/联网）、护栏拦截与敏感文件扫描，可导出 Markdown 报告
...
当前发布版本：**v0.3.0**（与 `src/localagent/__init__.py` / `la --version` 一致）。
...
> 发布到 PyPI 后可直接：`pipx install la-localagent==0.3.0` / `pipx upgrade la-localagent`
...
**当前召回分数（2026-07-14，`conv-26`，Mem0 hybrid + cross-encoder rerank，n=150）**：Hit@1 **0.433** / Hit@5 **0.627** / Hit@8 **0.673**。
...
命令:
  ...
  <command>
    ...
    memory         add|ingest|query|search|…  对话记忆写入/消费/查询/运维
    rag            add|ingest|search|…           文档知识库（Cold，不提取记忆）
...
叙事主线：**完全本地** → **长期多层次记忆（Mem0）** → **对话记忆 + 文档 RAG** → **本机 · 画像 · 联网三层打通**。
...
| **Warm** | Mem0（默认）或 JSON `memory_store` | 长期对话记忆 | ChatGPT 导入 · LA 对话提取 · `memory add` / `retain_memory` |
| **Cold** | Chroma + BM25（+ RRF） | 个人文档原文 | 仅 `LA rag add` / `rag ingest` — **不**提取记忆 |

Warm 与 Cold 刻意分离：对话沉淀为「关于你」的事实；文档保持可检索的原文。
...
（ADD / UPDATE / DELETE / NOOP）         │
        │                                ▼
        ▼                             Reflect（多跳检索 + LLM）
Mem0 / JSON 存储                      → 作答或继续追问检索
```

- **Retain**：从对话提取可沉淀事实；补全元数据；可选与近重复记忆做 consolidation
- **Recall**：混合检索 + 时间意图（`range` / `as_of_now` / `when_event` / …）、scoped soft boost、可选 cross-encoder / embed / LLM rerank
...
2. 打并推送同号 tag：`git tag v0.3.0 && git push origin v0.3.0`
