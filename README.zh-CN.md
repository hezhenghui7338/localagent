...
Python 3.10+ · macOS / Linux / Windows · [pipx](https://pipx.pypa.io/) · 当前 **v0.5.0**
...
无 API → `la setup -y`（按需装 Ollama，并按本机内存拉取合适模型；≥14GB 为 `qwen3.5:4b`）
...
- 未配置对话模型时，`la setup` 按**系统总内存**推荐模型（可用 `OLLAMA_MODEL` / `LA_OLLAMA_MODEL` 或 `config/model_servers.yaml` 覆盖）：
  ...
  | 系统内存 | 推荐模型 | 说明 |
  | --- | --- | --- |
  | &lt; 6 GB | `qwen2.5:0.5b`（Mini） | 可跑通 Chat/基础记忆；复杂 Agent/多工具较弱 |
  | 6–10 GB | `qwen2.5:1.5b` | 轻量日常问答 |
  | 10–14 GB | `qwen2.5:3b` | 过渡档 |
  | ≥ 14 GB | `qwen3.5:4b` | 当前主推质量档 |
