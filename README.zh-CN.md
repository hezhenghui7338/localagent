...
| **P1** | 低配机（任意 OS） | 总内存 4–8GB | Mini 档 `qwen3.5:0.8b`——能聊；多工具 Agent 较弱 |
| **P1** | 中高配（任意 OS） | ≥10GB | 主推 `qwen3.5:4b`；≥18GB 可上 `qwen3.5:9b` |
...
默认本地跑通；可选云端与联网。详解：[一键总结 · 新闻嗅探 · 一键润色 · Aware](#日常实用一键总结--新闻嗅探--一键润色--aware)。
...
版本：[Releases](https://github.com/hezhenghui7338/localagent/releases) / [Tags](https://github.com/hezhenghui7338/localagent/tags)。首次运行会建 `~/.localagent/`（Windows：`%USERPROFILE%\.localagent\`）。

```bash
la                 # = la chat
la setup           # 引导装 Ollama（可跳过）；按内存选模型
la setup -y
...
| 文档入库 `LA ingest doc` | 否 | Cold 必索引；长文可写 Warm 摘要 |
...
```bash
la aware status
la aware grant fs terminal browser apps -y   # 按源 opt-in；敏感源可去掉 -y 看确认
la aware tick --no-chat                      # 采集一轮 → Episode / suggestion
la aware suggestion                          # approve|reject（白名单：仅 ingest / summarize）
la aware                                     # 智能总结（当前 + 近 3h）→ aware>
la aware --detail --since 3h                 # 分源明细
```
...
| 7   | 联网搜索最近新闻             | `LA chat` 或 `/deepsearch`（默认无需 Key） |
| 8   | **纯本地运行** qwen3.5:4b | `LA chat --provider ollama`             |
| 9   | Agent 自动执行终端命令       | `LA chat` → 「统计当前项目代码行数」              |
| 10  | 审计报告（Ollama 零费用）     | `LA audit --since 7d`                   |
...
| 步骤 | 命令 | 展示能力 |
|------|------|----------|
| 写入演变链 | `LA ingest text` × 4 | Retain + 自动标题/标签/发生时间 |
...
首次运行任意 `LA` 命令时，若当前为 bash/zsh，会自动安装 Tab 补全（写入 `~/.zshrc` / `~/.bashrc`，并挂到 `.venv` 激活钩子）。PowerShell / cmd 暂跳过。之后 `source .venv/bin/activate`（或新开终端），`LA memory` / `LA rag` + Tab 即可提示子命令。
...
- 未配置对话模型时，`la setup` 按**系统总内存**推荐模型（可用 `OLLAMA_MODEL` / `LA_OLLAMA_MODEL` 或 `config/model_servers.yaml` 覆盖）：

  | 系统内存 | 推荐模型 | 说明 |
  | --- | --- | --- |
  | &lt; 6 GB | `qwen3.5:0.8b`（Mini） | 可跑通 Chat/基础记忆；复杂 Agent/多工具较弱 |
  | 6–10 GB | `qwen3.5:2b` | 轻量日常问答 |
  | 10–18 GB | `qwen3.5:4b` | 当前主推质量档 |
  | ≥ 18 GB | `qwen3.5:9b` | 高配质量档 |

- 若推荐模型未安装，LA 会改用本机已有对话模型（优先已加载的），仅在没有任何可用模型时才提示拉取
...
- Windows 上 `la news schedule` 暂不可用——请手动 `la news sync`（或用系统「任务计划程序」）

## <img src="assets/icons/config.svg" alt="" width="28" valign="middle"> 配置
...
日常以对话为主。外层命令与会话内 `/command` 同路径（如 `/ingest text` ≡ `LA ingest text`）。会话快捷方式：`/add` → `ingest text`，`/search` → `memory search`（会话内禁止再开 `/chat`）。
...
日常：
  ...
  la aware [status|grant|ungrant|tick|schedule|suggestion|paths|events]
                                # 本机感知 → aware>；--detail / --since / --no-chat
...
| **Warm** | Mem0（默认）或 JSON `memory_store`（+ 可选 SQLite 关系图） | 长期对话记忆 | ChatGPT 导入 · LA 对话提取 · `LA ingest text` / `retain_memory` |
| **Cold** | Chroma + BM25（+ RRF） | 个人文档原文 + 对话归档（摘要/轮次块） | `LA ingest doc` / `LA ingest kb`（文档）；ChatGPT / LA 对话 ingest 时先写 Cold |
...
# 结束后会打印「单元 / 集成」与「端到端」分栏，以及每个文件的通过数与模块说明
pytest
pytest -n0   # 串行（调试单测 / 减少 worker 噪音）
pytest --no-suite-summary   # 关闭分文件汇总

# 端到端：subprocess 调用 LA 命令（CI e2e-offline job 也会跑）
pytest tests/e2e -m e2e
pytest tests/e2e/test_la_perf.py -m e2e -n0   # 时长预算套件，建议串行

# 一次跑全量（单元 + e2e；覆盖默认的 marker 过滤）
pytest -m ""
...
## <img src="assets/icons/security.svg" alt="" width="28" valign="middle"> 安全与隐私
