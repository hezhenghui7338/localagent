...
- **长文档一键总结（Map-Reduce）** — 超长 PDF 的分层摘要；当前短文档路径已可用（见下）。
...
| 一键总结本地文档（默认进文档对话） | `la summarize <path>` · `sum>` 内 `/keep` 收藏；仅速读加 `--no-chat` |
| 新闻嗅探 / 今日简报 | `la news sync` → `la news brief`（TTY 交互浏览）；精读深聊 `r`；定时 `la news schedule on` |
| 一键润色文案（默认复制主推） | `la polish` / `/polish` · `--scene` / `--tone` / `--no-copy` |

> **日常三剑客**（最常用）：[一键总结](#日常实用一键总结--新闻嗅探--一键润色) · [新闻嗅探](#日常实用一键总结--新闻嗅探--一键润色) · [一键润色](#日常实用一键总结--新闻嗅探--一键润色)
...
- **文档知识库（RAG）**：软链导入个人文件（含 PDF），Chroma + BM25 混合检索
- **一键总结**：`la summarize` 对 txt / md / pdf / xlsx 输出最多三句话 + 带章节/页索引的要点，**默认进入 `sum>` 文档对话**；仅速读用 `--no-chat`（可多文件）；**默认不入库**，会话内 `/keep` 或 `--keep` 收藏到知识库（不会每次追问）
- **新闻嗅探**：`la news sync` 从 BestBlogs RSS 拉取精选；`la news brief` 在 TTY 下进入交互浏览器（↑↓ 切换、`o` 打开原文、`r` 精读并深聊）；`--no-ui` 一次性输出；`la news schedule on` 可配置早 8 点自动同步
- **一键润色**：`la polish` / `/polish` 识别邮件 / 朋友圈 / 简历 / 商务对话场景与态度后改写；主推 + 备选；**默认复制主推到剪贴板**
