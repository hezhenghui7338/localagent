...
def test_e2e_rag_add_status_search(la_env, tmp_path: Path):
    ...
    add = run_la(["rag", "add", str(doc)], env=la_env)
    assert add.returncode == 0
    assert "软链:" in add.stdout
...
def test_e2e_rag_ingest_skip_then_force(la_env, tmp_path: Path):
    ...
    skipped = run_la(["rag", "ingest"], env=la_env)
    assert skipped.returncode == 0
    assert "skipped" in skipped.stdout or "rag ingest" in skipped.stdout

    forced = run_la(["rag", "ingest", "--force"], env=la_env)
    assert forced.returncode == 0
    assert "updated" in forced.stdout or "new" in forced.stdout or "rag ingest" in forced.stdout
...
def test_e2e_rag_ingest_new_symlink_only(la_env, tmp_path: Path, la_data_dir: Path):
    ...
    result = run_la(["rag", "ingest"], env=la_env)
    assert result.returncode == 0
    assert "new" in result.stdout or "linked" in result.stdout or "rag ingest" in result.stdout
...
def test_e2e_rag_rebuild_after_reset(la_env, tmp_path: Path):
    ...
    rebuild = run_la(["rag", "rebuild"], env=la_env)
    assert rebuild.returncode == 0
    assert "rag rebuild" in rebuild.stdout
...
def test_e2e_rag_add_missing_file(la_env, tmp_path: Path):
    ...
    result = run_la(["rag", "add", str(missing)], env=la_env)
...
def test_e2e_rag_rebuild_idempotent(la_env, tmp_path: Path):
    ...
    assert run_la(["rag", "add", str(doc)], env=la_env).returncode == 0
    first = run_la(["rag", "rebuild"], env=la_env)
    second = run_la(["rag", "rebuild"], env=la_env)
    assert first.returncode == 0
    assert second.returncode == 0
    assert "rag rebuild" in first.stdout
...
def test_e2e_rag_ingest_help(la_env):
    result = run_la(["rag", "ingest", "--help"], env=la_env)
    assert result.returncode == 0
