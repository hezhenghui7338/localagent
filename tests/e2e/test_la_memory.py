...
def test_e2e_memory_help(la_env):
    ...
    # reflect 仍为 memory 子命令；一级 LA reflect 为兼容别名
    ok = run_la(["memory", "reflect", "x"], env=la_env)
...
def test_e2e_memory_ingest_chatgpt_file(la_env, tmp_path: Path):
    marker = "LocalAgentE2EIngestMarker2026"
    export = tmp_path / "conversations.json"
    export.write_text(
        json.dumps(
            minimal_chatgpt_export(
                conversation_id="conv-e2e-ingest",
                user_text=f"2026年7月我决定用 {marker} 管理个人记忆",
            ),
            ensure_ascii=False,
        ),
        encoding="utf-8",
    ...
    assert result.returncode == 0
    # Extraction may yield 0 without usable LLM; Cold indexing must still report chunks.
    assert "cold_chunks=" in result.stdout
    cold_m = re.search(r"cold_chunks=(\d+)", result.stdout)
    assert cold_m is not None and int(cold_m.group(1)) > 0

    search = run_la(["rag", "search", marker], env=la_env, timeout=60)
    assert search.returncode == 0
    assert marker in search.stdout
...
def test_e2e_reflect_smoke(la_env):
    """Offline-safe: command returns; may synthesize or report inability."""
    seed_memory(la_env, FACT_A)
    result = run_la(["reflect", "Mem0 相关决策是什么"], env=la_env, timeout=180)
    assert result.returncode == 0
    assert "综合推理" in result.stdout or result.stdout.strip()
...
def test_e2e_reflect_empty_bank(la_env):
    result = run_la(["reflect", "没有任何记忆时会怎样"], env=la_env, timeout=120)
    assert result.returncode == 0
    assert "未能从记忆与知识库中推理出答案" in result.stdout or result.stdout.strip()


def test_e2e_reflect_help(la_env):
    result = run_la(["reflect", "--help"], env=la_env)
    assert result.returncode == 0
