...
def test_e2e_add(la_env):
    result = run_la(["ingest", "text", "2026年7月决定使用 Mem0 作为记忆引擎"], env=la_env)
    assert result.returncode == 0
...
def test_e2e_add_rejects_short_text(la_env):
    result = run_la(["ingest", "text", "好的"], env=la_env)
...
def test_e2e_rag_add_and_ingest(la_env, tmp_path: Path):
    ...
    result = run_la(["ingest", "doc", str(doc)], env=la_env)
    assert result.returncode == 0
    ...
    result2 = run_la(["ingest", "kb"], env=la_env)
...
def test_e2e_rag_ingest_force(la_env, tmp_path: Path):
    ...
    run_la(["ingest", "doc", str(doc)], env=la_env)

    result = run_la(["ingest", "kb", "--force"], env=la_env)
    assert result.returncode == 0
    assert "updated" in result.stdout or "new" in result.stdout or "rag ingest" in result.stdout or "ingest kb" in result.stdout


def test_e2e_search_memory(la_env):
    run_la(["ingest", "text", "2026年7月决定使用 Mem0 作为记忆引擎"], env=la_env)
...
def test_e2e_reset_memory(la_env, tmp_path: Path):
    ...
    run_la(["ingest", "doc", str(doc)], env=la_env)
    run_la(["ingest", "text", "用户喜欢喝葡萄酒。"], env=la_env)
...
def test_e2e_tasks_list(la_env, tmp_path: Path):
    ...
    run_la(["ingest", "doc", "--background", str(doc)], env=la_env)
